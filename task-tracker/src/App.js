...
function App() {
  ...
  const fetchTasks = async (person) => {
    try {
      const response = await axios.get(`https://solo-tasks.vercel.app/api/tasks/${person}`);
  ...
  const fetchPersonInfo = async (person) => {
    try {
      const response = await axios.get(`https://solo-tasks.vercel.app/api/persons/${person}`);
  ...
  const markTaskCompleted = async (taskId) => {
    try {
      await axios.post(`https://solo-tasks.vercel.app/api/tasks/${selectedPerson}/${taskId}/complete`);
      fetchTasks(selectedPerson);
    } catch (error) {
